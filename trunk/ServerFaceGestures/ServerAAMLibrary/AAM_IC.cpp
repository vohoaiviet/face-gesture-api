				   ...
				   double shape_percentage /* = 0.95 */, double texture_percentage /* = 0.95 */)
{
	...
	__texture.Train(AllShapes, __paw, AllImages, texture_percentage);
				...
				int max_iter /* = 30 */, 	bool showprocess /* = false */)
{
	...
	for( iter = 0; iter < max_iter; iter++)
	{
		...
		if(!AAM_IC::IsShapeWithinImage(__current_s, image->width, image->height)){
			if( AAM_DEBUG_MODE ) fprintf(stderr, "ERROR(%s, %d): Shape out of image\n",
				__FILE__, __LINE__);
			return iter;
		}
	...
	if( AAM_DEBUG_MODE ) printf("AAM IC Fitting time cost %.3f millisec\n", t);
...
{
	printf("Reading the AAM-IC Model from file (%s)...", pModelFileName.c_str());

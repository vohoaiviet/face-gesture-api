				...
				CvMat* Texture, bool normalize)const
{
	...
	for(int i = 0, k = 0; i < __nPixels; i++, k+=3)
	{
		...
		AAM_PAW::GetBilinearPixel(data[k], data[k+1], data[k+2], image, x, y);
							...
							bool normalize)const
{
	...
	for(int i = 0, k = 0; i < __nPixels; i++, k+=3)
	{
		...
		X = (int)x;		Y = (int)y;		X1 = X+1;		Y1 = Y+1;
		s = x-X;		t = y-Y;		s1 = 1-s;		t1 = 1-t;
		...
		b1 = t1 * ltb + t * lbb;		b2 = t1 * rtb + t * rbb;
		g1 = t1 * ltg + t * lbg;		g2 = t1 * rtg + t * rbg;
		r1 = t1 * ltr + t * lbr;		r2 = t1 * rtr + t * rbr;
